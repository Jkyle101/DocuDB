...
import {
  FaFolder, FaFileAlt, FaChevronRight, FaPlus, FaUpload, FaTrash,
  FaArrowLeft, FaShareAlt, FaArrowsAlt, FaTh, FaList,
  FaFilePdf, FaFileWord, FaFileExcel, FaFileImage, FaFileArchive, FaFileVideo
} from "react-icons/fa";
...
export default function Home() {
  ...
  const iconByMime = useMemo(
    () => (mimetype) => {
      ...
      if (mimetype.includes("pdf")) return <FaFilePdf size={36} className="text-danger" />;
      if (mimetype.includes("word") || mimetype.includes("doc")) return <FaFileWord size={36} className="text-primary" />;
      if (mimetype.includes("excel") || mimetype.includes("spreadsheet")) return <FaFileExcel size={36} className="text-success" />;
      if (mimetype.includes("image")) return <FaFileImage size={36} className="text-warning" />;
      if (mimetype.includes("zip") || mimetype.includes("rar")) return <FaFileArchive size={36} className="text-muted" />;
      if (mimetype.includes("video")) return <FaFileVideo size={36} className="text-info" />;
  ...
  const deleteFolder = async (folder) => {
    ...
    await axios.delete(`${API}/folders/${folder._id}`, { params: { userId, role } });
    setFolders((s) => s.filter((f) => f._id !== folder._id));
  };

  const deleteFile = async (file) => {
    if (!window.confirm(`Delete file "${file.originalName}"?`)) return;
    await axios.delete(`${API}/files/${file._id}`, { params: { userId, role } });
    ...
    <div className="container-fluid py-3">
      ...
      <div className="d-flex justify-content-between align-items-center mb-3 flex-wrap gap-2">
        ...
        <div className="d-flex align-items-center gap-2 flex-wrap">
          <button className="btn btn-light" onClick={goUp} disabled={!breadcrumbs.length}>
            <FaArrowLeft />
          </button>
          <div className="d-flex align-items-center flex-wrap overflow-auto">
            <span className="fw-bold me-2">My Drive</span>
            {breadcrumbs.map((b) => (
        ...
        <div className="d-flex align-items-center gap-2 flex-wrap">
          ...
          <button className="btn btn-outline-secondary" onClick={() => setShowCreate(true)}>
            <FaPlus className="me-1" /> New Folder
          </button>
          <button className="btn btn-outline-success" onClick={() => setShowUpload(true)}>
            ...
            {files.map((file) => (
              <div key={file._id} className="col-6 col-sm-4 col-md-3 col-lg-2">
                <div className="card p-3 h-100 text-center shadow-sm">
                  ...
                  <div className="text-truncate fw-semibold">{file.originalName}</div>
                  <div className="d-flex justify-content-center gap-2 mt-2 flex-wrap">
                    <a
          ...
          <table className="table table-hover align-middle">
            ...
            <tbody>
              ...
              {files.map((file) => (
                <tr key={file._id}>
                  <td>
                    {iconByMime(file.mimetype)} <span className="ms-2">{file.originalName}</span>
                  </td>
                  <td>{file.mimetype}</td>
